...
export const POST = asyncHandler(async (request?: NextRequest): Promise<NextResponse> => {
    ...
    if (avatar instanceof File === false) {
        return NextResponse.json(new ApiError(400, "avatar image must be a file type"))
    }
    ...
    try {
        await writeFile(
            avatarURL,
            buffer
        );   //this always returns void hence need trycatch
    } catch (error) {
        return NextResponse.json(new ApiError(500, "Failed to upload avatar"));
    }
    ...
    return NextResponse.json(new ApiResponse(200, "Avatar uploaded successfully"));
})